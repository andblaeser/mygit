...
def repo_create(path):
    ...
    assert (repo_dir(repo, "tags", mkdir=True))
    assert (repo_dir(repo, "heads", mkdir=True))
...
def kvlm_parse(raw, start=0, dct=None):
    if not dct:
        ...
        # You CANNOT declare the argument as dct=OrderedDict() or all call to the
        # functions will endlessly grow the same dict.
    ...
    # find returns -1), we assume a blank line. A blank line means the remainder
    # of the data is the message.
    if (spc < 0) or (nl < spc):
        assert (nl == start)
        dct[b''] = raw[start+1:]
        return dct
...
def cmd_ls_tree(args):
    ...
    for item in obj.items:
        print("{0} {1} {2}\t{3}".format(
            ...
            # Git ls-tree displays the type of the object pointed to. We can too.
...
def ref_list(repo, path=None):
    ...
    # Git shows refs sorted. To do the same, we use an OrderedDict
...
argsp.add_argument("--mygit-type", metavar="type", dest="type", choices=[
                   "blob", "commit", "tag", "tree"], default=None, help="Specify the expected type")
...
class GitIndexEntry(object):
    ...
    mode_type = None
    """The object's type, either b1000 (regular), b1010 (symlink), b1110 (gitlink)."""
    mode_perms = None
